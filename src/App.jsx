...
//   alert(`Product ${listProduct.productName} Dengan harga ${listProduct.productPrice}`);
// }

function App() {
  const handleClick = (productName,productPrice) => {
    alert(`Product ${productName} Dengan harga ${productPrice} Beli sekarang`);
  };
  return (
    <>
      <Header/>
      ...
      {listProduct.map((l) => (
        <>
          <Tombol onClick={() => handleClick(l.productName,l.productPrice)}>
          <img key={l.id} src={l.urlImage}/>
          <p key={l.id}>
            {l.productName} 
            <br />
            {l.productPrice}
